...
describe('Router', () => {
  ...
  it('should route to custom OpenAI-compatible providers in the fallback chain', () => {
    const db = getDb();
    const platform = 'custom:testlocal';

    db.prepare(`
      INSERT INTO custom_providers (platform, name, base_url)
      VALUES (?, ?, ?)
    `).run(platform, 'Local vLLM', 'http://127.0.0.1:8000/v1');
    ...
    const modelResult = db.prepare(`
      ...
      VALUES (?, ?, ?, 1, 1, 'Custom', NULL, NULL, NULL, NULL, 'custom', 8192)
    `).run(platform, 'local-model', 'Local Model');

    db.prepare(`
      INSERT INTO fallback_config (model_db_id, priority, enabled)
      VALUES (?, 1, 1)
    `).run(modelResult.lastInsertRowid);

    const customKey = encrypt('local-secret');
    db.prepare(`
      INSERT INTO api_keys (platform, label, encrypted_key, iv, auth_tag, status, enabled)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(platform, 'local', customKey.encrypted, customKey.iv, customKey.authTag, 'healthy', 1);
